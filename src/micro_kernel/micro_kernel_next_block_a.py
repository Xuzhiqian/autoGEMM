...
def micro_kernel_next_block_a_load_data(line, col,
    ...
    if (REG_BLOCK_TRANS_FLAG == 0 and ( # unknown constraints
        ...
        if is_first_line and is_first_col: # 第0行第0列
            for real_line in range(real_lines):
                if ( # unknown constraints
                    ...
                    (real_line >= get_permuted_lines(real_lines, VEC_REG_A_LEN) or 2 * real_lines <= VEC_REG_A_LEN)
                ):
                    actual_line = get_permuted_line(real_line, real_lines, VEC_REG_A_LEN) if is_A_even else real_line
                    x_A_idx = get_x_A_idx(actual_line, LINES)
        ...
        if ( # unknown constraints
            ...
            line < get_permuted_lines(real_lines, VEC_REG_A_LEN)
        ):
            actual_line = get_permuted_line(line, real_lines, VEC_REG_A_LEN) if is_A_even else line
            x_A_idx = get_x_A_idx(actual_line, LINES)
            vector_A_idx = get_vector_A_idx(actual_line, A_odd_flag^1, vector_scroll_A)
            code_str += load_A_data_and_offset(vector_A_idx, x_A_idx)
