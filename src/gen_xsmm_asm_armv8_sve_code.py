...
"""
    cc_code += laf_asm_code(M, N, K, lda, ldb, ldc, 2, UNROLL_K, NR_MAIN, MRSA_FLAG, with_bias = 0)
    cc_code += f"""
}}
void small_gemm_with_bias(const float *A, const float *B, float *C, int lda, int ldb, int ldc) {{
"""
    cc_code += laf_asm_code(M, N, K, lda, ldb, ldc, 2, UNROLL_K, NR_MAIN, MRSA_FLAG, with_bias = 1)
    cc_code += f"""
}}
}}
